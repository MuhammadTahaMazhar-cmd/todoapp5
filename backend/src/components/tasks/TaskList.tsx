      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 p-4 bg-white dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['all', 'active', 'completed'] as TaskFilter[]).map((f) => (
              ...
              >
                {f === 'active' ? 'Active' : f} ({f === 'all' ? stats.total : f === 'active' ? stats.active : stats.completed})
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">
            No {filter === 'all' ? '' : filter === 'active' ? 'active' : filter} tasks found
          </h3>
          <p className="text-slate-600 dark:text-slate-400 max-w-sm">
            {filter === 'all'
              ? 'Add your first task to get started. Organize your work and boost your productivity!'
              : `No ${filter === 'active' ? 'active' : filter} tasks at the moment.`}
          </p>
        </div>
      </div>
    );
