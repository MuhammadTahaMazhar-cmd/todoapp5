...
export function TaskTable({ tasks: propTasks }: TaskTableProps) {
  ...
  const handleEdit = (task: Task) => {
    ...
    if (task.id.startsWith('temp-')) {
      toast.warning('Task is still being added. Please wait a moment.');
      return;
    }
    setEditingTask(task);
                    ...
                    <td colSpan={8} className="px-4 py-12 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-slate-600 dark:text-slate-400">
                          No {filter === 'all' ? '' : filter === 'active' ? 'incomplete' : filter} tasks found
                        </p>
                      </div>
                    </td>
                  </tr>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <Button
                  ...
                  className="flex-1"
                >
                  {viewingTask.completed ? 'Mark as Incomplete' : 'Mark as Completed'}
                </Button>
                <Button
                  variant="secondary"
                  onClick={() => {
